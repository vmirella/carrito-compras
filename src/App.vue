  ...
  <v-app>
    <v-toolbar app class="grey darken-3">
      <v-toolbar-title class="headline text-uppercase">
        <router-link to="/" style="color:#DD2C00; font-weight: 700"
          >marca perú</router-link
        >
      ...
      <v-toolbar-items class="hidden-sm-and-down">
        <router-link
          to="/"
          class="body-2 font-weight-bold white--text v-btn v-btn--flat theme--light"
          flat
          >INICIO</router-link
        >
        <router-link
        ...
        <v-menu offset-y>
          <template v-slot:activator="{ on }">
            <v-btn flat v-on="on">
              ...
              <v-icon class="body-2 font-weight-bold white--text"
                >arrow_drop_down</v-icon
              >
            </v-btn>
          </template>
          <v-list>
            <v-list-tile v-for="(item, index) in categories" :key="index">
              <v-list-tile-title>
                <router-link
    ...
    <v-footer dark fixed height="auto">
      <v-flex secondary lighten-2 py-3 text-xs-center white--text xs12>
        Desarrollado por:
        <a
          href="httpss://vmirella.github.io/"
          target="_blank"
